<template>
  <div>
    <h1 id="셀레니움-크롬드라이버-버전-이슈-해결방법-this-version-of-chromedriver-only-supports-chrome-version" tabindex="-1">
      <a class="header-anchor" href="#셀레니움-크롬드라이버-버전-이슈-해결방법-this-version-of-chromedriver-only-supports-chrome-version" aria-hidden="true">#</a> 셀레니움 크롬드라이버 버전 이슈 해결방법
      (This version of ChromeDriver only supports Chrome version)
    </h1>
    <div class="custom-container tip">
      <p class="custom-container-title">목표</p>
      <p>셀레니움을 사용하다보면 아래와 같은 에러를 자주 만나는데 해결방법에 대해 알아보자</p>
    </div>
    <div class="language-bash ext-sh line-numbers-mode">
      <pre v-pre class="language-bash"><code>session not created: This version of ChromeDriver only supports Chrome version <span class="token number">105</span>
Current browser version is <span class="token number">107.0</span>.5304.87 with binary path /Applications/Google Chrome.app/Contents/MacOS/Google Chrome
    ...
    <p>
      <a href="https://chromedriver.chromium.org/downloads" target="_blank" rel="noopener noreferrer">https://chromedriver.chromium.org/downloads<ExternalLinkIcon /></a>
    </p>
    ...
    <p>에러메세지에서 나와있는 버전의 크롬 드라이버를 선택해준다.</p>
    ...
    <p>사용하는 환경에 맞는 파일을 다운로드해준다. 나는 M1 mac을 사용중이니 <code v-pre>chromedriver_mac_arm64.zip</code>을 다운로드 받아준다.</p>
    ...
    <p>이렇게 설치가 성공적으로 됐다는 메세지가 나오면 성공이다. 이제 설치를 마친 크롬드라이버를 환경변수 폴더로 이동시켜주어야한다.</p>
    <div class="language-text ext-text line-numbers-mode">
      <pre v-pre class="language-text"><code>cd /usr/local/bin/
</code></pre>
    ...
    <p>기존 크롬드라이버 제거</p>
    <div class="language-text ext-text line-numbers-mode">
      <pre v-pre class="language-text"><code>sudo rm chromedriver
</code></pre>
      <div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div>
    </div>
    <p>크롬드라이버 이동</p>
    <div class="language-text ext-text line-numbers-mode">
      <pre v-pre class="language-text"><code>sudo mv chromedriver /usr/local/bin
</code></pre>
      <div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div>
    </div>
  </div>
