<template>
  <div>
    <h1 id="netlify에-nextjs-프로젝트-배포하는-두-가지-방법-next-out" tabindex="-1">
      <a class="header-anchor" href="#netlify에-nextjs-프로젝트-배포하는-두-가지-방법-next-out" aria-hidden="true">#</a> Netlify에 Nextjs 프로젝트 배포하는 두 가지 방법 (.next , out)
    </h1>
    <p>Nextjs 프로젝트는 두가지 형식의 사이트로 사용되는데요. 첫번째는 <code v-pre>.next</code> 디렉토리의 일반적인 Nextjs 프로젝트 두번째는 <code v-pre>out</code> 디렉토리로 생성되는 정적 HTML 웹사이트입니다.</p>
    ...
    <p>Netlify를 통해서 Nextjs를 배포할 때는 위 두가지 중 어떤 것으로 배포하냐에 따라 빌드 설정이 달라지게 됩니다. 빌드 설정을 잘못하게 되면 아래와 같은 에러가 발생합니다.</p>
    ...
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/1.png" alt="" /></p>
    <div class="language-bash ext-sh line-numbers-mode">
      <pre v-pre class="language-bash"><code><span class="token number">12</span>:48:25 PM:   Error message
...
<span class="token number">12</span>:48:25 PM:   The publish directory should be <span class="token builtin class-name">set</span> to <span class="token string">"out"</span>, and you should <span class="token builtin class-name">set</span> the environment variable NETLIFY_NEXT_PLUGIN_SKIP to <span class="token string">"true"</span><span class="token builtin class-name">.</span>
    ...
    <p>위 에러는 <code v-pre>next export</code> 명령어를 통해 static 웹사이트를 만들었는데 netlify의 publish directory가 <code v-pre>.next</code> 로 되어있어서 발생한 오류인데요.</p>
    ...
    <p>
      정적웹사이트를 배포하는 과정에서는 Netlify에서 Nextjs 플러그인을 빌드할 필요가 없기 때문에 skip에 true를 넣어주는 것이죠.<a
    ...
    <p>
      netlify의 환경변수에서 <code v-pre>NETLIFY_NEXT_PLUGIN_SKIP</code>를 살펴보면 아래와 같이 설명되어있습니다.
      <strong>이 값을 true로 변경하면 next.js 런타임을 사용하지 않는 것이고 next export를 이용해서 static html를 생성한다고 나와있습니다.</strong>
    </p>
    <p>::: NETLIFY_NEXT_PLUGIN_SKIP: when set to true for a Next.js site, the build doesn’t use the Next.js Runtime. Use this variable with projects that generate static HTML using next export. :::</p>
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/3.png" alt="" /></p>
    ...
    <p><ins class="adsbygoogle" style="display: block" data-ad-client="ca-pub-4877378276818686" data-ad-slot="9743150776" data-ad-format="auto" data-full-width-responsive="true"></ins></p>
    <component is="script"> (adsbygoogle = window.adsbygoogle || []).push({}); </component>
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/4.png" alt="" /></p>
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/7.png" alt="" /></p>
    ...
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/6.png" alt="" /></p>
    ...
    <div class="language-json ext-json line-numbers-mode">
      <pre
        v-pre
        class="language-json"
      ><code><span class="token property">"build"</span><span class="token operator">:</span> <span class="token string">"next build &amp;&amp; next export"</span><span class="token punctuation">,</span>
</code></pre>
    ...
    <p><img src="@source/docs/SEO/2022-11-28-Netlify에-Nextjs-프로젝트-out-디렉토리로-배포하는-방법/img/8.png" alt="" /></p>
